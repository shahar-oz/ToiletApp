...
namespace ToiletApp.Services
{
    public class ToiletAppWebAPIProxy
    {
        ...
        */
        ...
        public async Task<SaninManInfo?> AddSanitMan(SaninManInfo sanitman)
        {
            //Set URI to the specific function API
            string url = $"{this.baseUrl}addSanitMan";
            try
            {
                //Call the server API
                string json = JsonSerializer.Serialize(sanitman);
                StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
                HttpResponseMessage response = await client.PostAsync(url, content);
                //Check status
                if (response.IsSuccessStatusCode)
                {
                    ...
                    SaninManInfo? result = JsonSerializer.Deserialize<SaninManInfo>(resContent, options);
                    return result;
                }
                else
                {
